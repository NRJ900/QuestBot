...
class DiscordQuestBotGUI:
    ...
    def setup_ui(self):
        ...
        subtitle = Label(
            title_frame,
            text="Automatic Quest Completion for DiscordPTB",
            font=("Segoe UI", 11),
            bg=self.card_bg,
            fg=self.muted_text
        )
        ...
        self.log_app("Ready! Click 'Start Modified DiscordPTB' to begin", "info")
    ...
    def start_modified_discord(self):
        ...
        def inject_thread():
            try:
                ...
                self.log_app("🚀 Starting modified DiscordPTB injection...", "info")
                
                exe, resources = self.find_discord()
                if not exe:
                    self.log_app("❌ DiscordPTB not found!", "error")
                    self.update_status("Error", self.danger_color)
                    return
                
                self.log_app(f"✅ Found DiscordPTB", "success")
                ...
                if os.path.exists(asar_backup):
                    ...
                    if killed > 0:
                        self.log_app(f"🛑 Closed {killed} DiscordPTB process(es)", "info")
...
"""
                ...
                self.log_app("✅ MODIFIED DISCORDPTB STARTED!", "quest")
                self.log_app("═" * 50, "quest")
    ...
    def start_original_discord(self):
        ...
        def start_thread():
            try:
                ...
                self.log_app("🔓 Starting Unmodified DiscordPTB...", "info")
                ...
                subprocess.Popen([exe])
                self.log_app("✅ Unmodified DiscordPTB started", "success")
                
                self.update_status("Running", self.accent_color)
                self.update_discord_status(True, modified=False)
                
                self.log_discord("ℹ️  Unmodified DiscordPTB - No quest bot active", "quest_info")
    ...
    def stop_discord(self):
        ...
        def stop_thread():
            try:
                self.log_app("🛑 Stopping DiscordPTB ...", "warning")
                ...
                if killed > 0:
                    ...
                    self.log_app("ℹ️  DiscordPTB was not running", "info")
                ...
                # Step 4: ONLY delete the backup after ALL verifications pass
                try:
                    os.remove(asar_backup)
    ...
    def start_monitoring(self, exe_path, asar_file, asar_backup):
        ...
        def monitor():
            self.log_app("👁️ Monitoring DiscordPTB process...", "info")
            time.sleep(5)
            ...
            self.log_app("🛑 DiscordPTB has been closed, You can safely quit this app now.", "warning")
            time.sleep(1)
            ...
            try:
                ...
                self.log_app("🔄 Auto-restoring original DiscordPTB...", "info")
